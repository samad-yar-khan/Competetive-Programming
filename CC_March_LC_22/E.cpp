...
ll solve(vector<int>&vec , int i , vector<int>final,int n,int si,int ei){
    cout<<i<<" ";
    if(i>ei){
        if(final.size()>1)
        {
    ...
    vector<int>dup=final;
    dup.push_back(vec[i]);
    return max(solve(vec,i+1,final,n,si,ei),solve(vec,i+1,dup,n,si,ei));
}
...
int main(){
    ...
    while(T--){
       ...
       vector<int>vec(N+1,0);
        ...
        for(int i =0;i<Q;i++){
            vector<int> ans;
            cout<<solve(vec,qs[i][0],ans,N,qs[i][0],qs[i][1])<<"\n";
        }
