...
int main(){
    ...
    while(T--){
        ...
        int num=0;
        cin>>num;
        vector<int>boxes(num,0);
        for(int i=0;i<num;i++){
            cin>>boxes[i];
        }

        int unitPBsize =0;cin>>unitPBsize;
        vector<int>unitsPerBox(unitPBsize,0);
         for(int i=0;i<unitPBsize;i++){
            cin>>unitsPerBox[i];
        }
        int truckSize =0;cin>>truckSize;

        //solvd
        ll ans =0;
        vector<pair<ll,ll>>vec;
        for(int i =0;i<num;i++){
            ll prods = unitsPerBox[i];
            ll boxs = boxes[i];
            vec.push_back({prods,boxs});
        }
        sort(vec.begin(),vec.end());
        reverse(vec.begin(),vec.end());
        for(int i =0;i<num && truckSize >0 ;i++){
            while(vec[i].second>0 && truckSize>0){
                ans+=vec[i].first;
                vec[i].second--;
                truckSize--;
            }
