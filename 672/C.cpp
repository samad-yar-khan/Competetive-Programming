...
ll solve(vector<ll>&vec,int n, vector<vector<ll>>&dp,bool state,int ci){
    ...
    if(state){
        ...
        ll case1 = solve(vec,n,dp,!state,ci+1)+vec[ci];
        ll case2 = solve(vec,n,dp,state,ci+1);
        dp[ci][s]=max(case1,case2);
        ans = dp[ci][s];
    }else{
